...
namespace ZTNAMESPACE
{
    ...
    UINT LampGroup::NextState()
    {
        ...
        while (++st!=m_State)
        {   //判断条件，状态一个个循环，遇到相同状态则不再判断。
            cursor=cursor<<1;
            
            if (8 == st)
            {     //如果state超过BYTE的位数，那么复位从0开始判断
                st = 0;
                cursor = 1;    //从bit1开始判断
            }
            if (cursor&sel)
...
#pragma mark -
    ...
    {
        if (0 == m_Lamps.size() || 0 == m_Lddout)
        {  //no lamps or default group
            return ZTTRUE;
        }
    ...
    ZTBOOL LampGroup::HitTest(POINT point)
    {
        return ZTFALSE;
    }
    
    void LampGroup::GenerateBinary(BYTE *&pByte, INT &length)
    {
        length = 8*m_Lamps.size();
        pByte = new BYTE[length];
        bzero(pByte, length);
        
        INT offset = 0;       //记录当前路灯数据的偏移
        SHORT *pShort = ZTNULL;
        list<Lamp*>::const_iterator it = m_Lamps.begin();
        while (it != m_Lamps.end())
        {
            BYTE *pLampByte = ZTNULL;
            INT len = 0;
            
            (*it)->GenerateBinary(pLampByte, len);
            
            memcpy(pByte+offset, pLampByte, len);
            pShort = (SHORT *)(pByte+offset+6);
            *pShort = MAKESHORT(m_State, m_Lddout);
